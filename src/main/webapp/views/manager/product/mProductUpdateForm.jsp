...
<%@page import="com.formypet.common.model.vo.Attachment"%>
	...
	<main id="main" class="main">
		...
		<section class="section">
			<div class="row">
				<div class="col">
					<div class="card">
						<div class="card-title">상품수정</div>
						<form action="<%=contextPath%>/update.mpro" method="post" enctype="multipart/form-data">
							<input type="hidden" name="no" value="<%=p.getProNo()%>">
							<input type="hidden" name="image" value="<%=p.getProMainImage()%>">
							<div class="product-bar">
								<div class="product-img d-flex justify-content-center">
									<img src="<%=p.getProMainImage() %>" class="img_preview" style="width: 80%">
								</div>
								<div class="col" style="margin: auto">
									<table class="product-info">
										<tr>
											<th>상품명</th>
											<td><input type="text" class="form-control" name="title" required value="<%=p.getProName()%>"></td>
										</tr>
										<tr>
											<th>상품코드</th>
											<td class="d-flex">
										...
										<tr>
											<th>가격</th>
											<td><input type="text" class="form-control" name="price" required value="<%=p.getPrice()%>"></td>
										</tr>
										<tr>
											<th>할인율</th>
										...
										<tr>
											<th>재고</th>
											<td><input type="text" class="form-control" name="stock" readonly value="<%=p.getStock()%>"></td>
										</tr>
										<tr id="add_list">
											<th>키워드</th>
											<td id="select_list">
												<select id="keyword_code" class="form-select me-2" name="keyword">
													<optgroup label="간식">
													...
													<optgroup label="용품">
														...
														<option value="급수/급식기">급수/급식기</option>
														<option value="기타">기타</option>
													</optgroup>
											</select>
											</td>
											<td><button type="button" class="btn btn-outline-secondary" id="add">+</button></td>
										</tr>
									</table>
								</div>
							</div>
							...
							<div class="product-img d-flex justify-content-end mt-3">
								<label for="file" class="btn btn-outline-secondary me-5">파일선택</label> 
									<input type="file" accept="image/*" name="upfile1" id="file" style="display: none;" onchange="loadImg(0);">
							</div>
							<!-- 이미지 등록 버튼 -->
							<div class="ms-5">
								상품상세 이미지 <input name="upfile2" class="form-control product-img" type="file" id="formFile" style="width: 40%;">
								<%for(Attachment count : at){%>
									<%if(count.getFileLevel() == 2){%>
									<%=count.getOriginName() %>
									<%} %>
								<%} %>
							</div>
							...
							<div class="d-flex justify-content-end me-5 mb-5">
								<button type="submit" class="btn btn-secondary event-btn">수정하기</button>
							</div>
							<!-- 등록하기 버튼 -->
							<script>
								/* 삭제버튼 상품 코드 테이블 지우기 */
								...
								$(document).ready(function(){
									//대분류
									let cate = '<%=p.getProCateCode() == null ? "" : p.getProCateCode() %>';
									$("#cate_code").val(cate).prop("selected", true);
				            		//대분류에 따른 리스트 변화
									changeList();
				            		
				            		//중분류
									let midd = '<%=p.getProMiddNo() %>';
				            		$("#midd_code").val(midd).prop("selected", true);
				            		
				            		//키워드 적용
				            		let keyword = '<%=p.getKeyword()%>';
				            		let count = keyword.split(",");
				            		if(count.length > 1){
					            		for(let i = 0; i < count.length - 1; i++){
					            			addList();
					            		}
					            		$("select[name='keyword']").each(function(index, element){
						            		$("select[name='keyword']").eq(index).val(count[index]).prop("selected", true);
					            		})
				            		} else {
				            			$("#keyword_code").val(keyword).prop("selected", true);
				            		}
				            		//메인 이미지 적용
									...
									$("#add").click(function addList(){
										...
										if(code == "D"){
											var tr = '<tr id="add_list">'
												   ...
												   + '</select>'
												   + '</td>'
												   + '<td><button type="button" id="delBtn" class="btn btn-outline-danger">-</button></td>'
												   + '</tr>';
											$("#add_list").after(tr);
								...
								function changeList(){
									...
									if(code == "D"){
										var tr = '<select id="keyword_code" class="form-select me-2" name="keyword">'
											   + '<optgroup label="사료">'
											   + '<option value="퍼피" selected>퍼피</option>'
											   + '<option value="어덜트">어덜트</option>'
											   + '<option value="시니어">시니어</option>'
										...
										var tr = '<select id="keyword_code" class="form-select me-2" name="keyword">'
											   ...
											   + '<option value="저키">저키</option>'
											   + '<option value="음료">음료</option>'
											   + '<option value="영양기능">영양/기능</option>'
											   + '<option value="처방식">처방식</option>'
											   + '<option value="수제간식">수제간식</option>'
											   + '</optgroup>'
											   + '<optgroup label="용품">'
