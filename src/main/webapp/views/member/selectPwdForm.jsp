...
.pwd-search-wrapper > h2{
    font-size: 24px;
    text-align: center;
    margin-bottom: 40px;
}

#pwd-search-form > input[type="submit"], #userId{
    width: 400px;
    height: 48px;
    padding: 0 10px;
    box-sizing: border-box;
...
#pwd-search-form > input::placeholder{
    color: #818080;
}

#pwd-search-form > input[type="submit"]{
    font-size: 16px;
   ...
   <!-- header end -->
      ...
      <section class="main_content">
		 ...
		 $(document).ready(function() {
		        ...
		        if (errorMsgValue === "noUser") {
		            ...
		            $('#selectFailureModal').on('hidden.bs.modal', function () {
		                ...
		                window.location.replace("<%= request.getContextPath() %>/selectPwdForm.me");
		            });
		            
		        } else if (errorMsgValue === "userFound") {
		            var userId = '<%= request.getAttribute("userId") %>';
		            var userPwd = '<%= request.getAttribute("userPwd") %>';
		            $('#selectFailureModal .modal-body').html(userId + "님의 비밀번호는 [ " + userPwd + " ] 입니다.<br>로그인 페이지로 이동합니다.");
		            $('#selectFailureModal').modal('show');
		            $('#selectFailureModal').on('hidden.bs.modal', function () {
		                // 모달이 닫힐 때 로그인 페이지로 이동
        ...
        <div class="input_group">
          <form method="post" action="<%= contextPath %>/selectPwd.me" id="pwd-search-form" onsubmit="validateForm()">
              <input type="text" name="userId" placeholder="아이디" id="userId"><br>
              <input type="text" name="email" placeholder="이메일 주소" id="email"><br>
              <p style="font-size: 12px;">
                &nbsp;* 포마이펫 가입 시 사용하신 이메일 주소를 입력해주세요. <br>
                &nbsp;&nbsp;&nbsp;해당 이메일로 임시 비밀번호를 발송합니다.
              </p>

              <input type="submit" value="이메일(E-mail)로 전송"><br>
			    ...
			    <div class="modal-dialog" role="document">
			        <div class="modal-content">
			            <div class="modal-header">
			                <h5 class="modal-title" id="selectFailureModalLabel">비밀번호 찾기</h5>
