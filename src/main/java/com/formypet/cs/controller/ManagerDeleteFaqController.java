...
/**
 * Servlet implementation class ManagerDeleteFaqController
 */
@WebServlet("/delete.mfq")
public class ManagerDeleteFaqController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ManagerDeleteFaqController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new FaqService().deleteFaq(noArr);
		...
		if(result > 0) {
			...
			// 글상태 변경 요청 실패
			// 실패 => 에러페이지 + 에러문구 "FAQ 삭제 실패" => 포워딩
			//request.setAttribute("errorMsg", "FAQ의 삭제에 실패했습니다.");
