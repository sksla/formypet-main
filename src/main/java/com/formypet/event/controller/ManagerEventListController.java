...
/**
 * Servlet implementation class ManagerEventListController
 */
@WebServlet("/list.mev")
public class ManagerEventListController extends HttpServlet {
     ...
     */
    public ManagerEventListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PageInfo pi = new PagingTemplate().Paging(request, "page", listCount, 5, 6);
		
		//System.out.println(pi);
		
		// * 사용자가 요청한 페이지에 보여져야될 게시글 목록
		List<Event> list = new EventService().selectList(pi);
		...
		request.getRequestDispatcher("/views/manager/event/mEventList.jsp").forward(request, response);
