...
/**
 * Servlet implementation class MypageAddressChangeController
 * 기본배송지와 서브배송지 변경하는 서블릿
 */
@WebServlet("/MypageAddressUpdate")
public class MypageAddressUpdateController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MypageAddressUpdateController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 이곳에 기본배송지 변경 클릭 기능 구현
		int result = new MypageService().updateAddress(userNo, deliveryNo);
        if (result > 0) {
            System.out.println("기본배송지 설정 완료");
        } else {
            System.out.println("기본배송지 설정 실패");
        }
