...
/**
 * Servlet implementation class ManagerStockListController
 */
@WebServlet("/list.msto")
public class ManagerStockListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ManagerStockListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean check = false;
		int listCount = new ProductService().selectStockListCount();
		PageInfo pi = new PagingTemplate().Paging(request, "page", listCount, 5, 10);

		// * 사용자가 요청한 페이지에 보여져야될 게시글 목록
		List<Stock> list = new ProductService().selectStockList(pi);
