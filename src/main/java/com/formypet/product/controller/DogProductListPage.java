...
/**
 * Servlet implementation class DogProductListPage
 */
@WebServlet("/list.d")
public class DogProductListPage extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DogProductListPage() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new ProductService().selectKindCount(kind);
		PageInfo pi = new PagingTemplate().Paging(request, "page", listCount, 5, 16);

		// * 사용자가 요청한 페이지에 보여져야될 게시글 목록
		List<Product> list = new ProductService().selectProductList(kind,pi);
		
		// 응답페이지 => /views/board/boardList.jsp
		// 응답데이터 => 사용자가 요청한 페이지상에 보여져야될 게시글 목록, 페이징바를 만들기위한 애들
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.getRequestDispatcher("/views/product/dogProductPage.jsp").forward(request, response);;
